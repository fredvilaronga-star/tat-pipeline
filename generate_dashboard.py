...
TYPE_COLOR = {"Email":"#4f7ee6","WhatsApp":"#12c98a","Reunião":"#22b8a6","Diagnóstico":"#a06cf0",
              "One-pager":"#f2a900","Nota":"#6b7a90","Ligação":"#4f7ee6"}
...
.board{{display:flex;gap:9px;padding:16px 18px;align-items:flex-start}}
.col{{flex:1 1 0;min-width:0;background:var(--s1);border:1px solid var(--b);border-radius:11px;padding:8px}}
@media(max-width:900px){{.board{{overflow-x:auto}}.col{{flex:0 0 200px}}}}
...
.sec{{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:var(--mut);margin:16px 0 10px;display:flex;align-items:center;gap:8px}}
.sec::after{{content:'';flex:1;height:1px;background:var(--b)}}
.tlw{{position:relative;padding-left:16px}}
.tlw::before{{content:'';position:absolute;left:4px;top:4px;bottom:4px;width:2px;background:var(--b)}}
.ev{{position:relative;margin-bottom:12px}}
.ev::before{{content:'';position:absolute;left:-14px;top:3px;width:9px;height:9px;border-radius:50%;background:var(--blue);border:2px solid var(--s1)}}
.ev-d{{font-size:9.5px;color:var(--mut);font-weight:700;text-transform:uppercase;letter-spacing:.3px}}
.ev-n{{font-size:12px;margin-top:2px;line-height:1.5}}
...
<div class="hdr">
  <div><div class="ht">🛩️ TAT WaterLine · Pipeline</div><div class="hs">Funil comercial · clique num card · confidencial · sem valores (US$)</div></div>
...
<div class="ftr">Gerado automaticamente a partir do Notion · atualização diária · TAT Operating · <b>Confidencial — uso interno.</b> Não inclui valores (US$).</div>
...
function tl(a){{
  ...
  let rows=ev.map(e=>{{
    const c=TY[e.t]||'#6b7a90';
    return '<div class="ev"><div class="ev-d">'+(e.d?fmt(e.d):'—')+'</div>'
